...
if (!isset($TEMPLATE)) {
  ...
  if ($format === 'json') {
    ...
    if (count($_GET) <= 1) {
      // Usage
      print str_replace("\\'", '"', json_encode($GEOSERVE_PLACES));
    } else {
      try {
...
<?php print $GEOSERVE_PLACES['description']; ?>
...
<pre>
  <code><?php print $GEOSERVE_PLACES['url']; ?></code>
</pre>

<h3>Parameters</h3>
<?php foreach ($GEOSERVE_PLACES['parameters'] as $title=>$info) : ?>
...
<h3>Notes</h3>
<?php foreach ($GEOSERVE_PLACES['notes'] as $note) : ?>
...
<?php foreach ($GEOSERVE_PLACES['output'] as $title=>$output) : ?>
...
<ul>
  <?php foreach ($GEOSERVE_PLACES['examples'] as $example) : ?>
    <li>
