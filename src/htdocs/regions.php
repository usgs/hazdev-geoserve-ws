...
if (!isset($TEMPLATE)) {
  ...
  if ($format === 'json') {
    ...
    if (count($_GET) <= 1) {
      // Usage
      print str_replace("\\'", '"', json_encode($GEOSERVE_REGIONS));
    } else {
      try {
...
<?php print $GEOSERVE_REGIONS['description']; ?>

<h2 id="request">Request</h2>

<h3>Syntax</h3>
<pre>
  <code><?php print $GEOSERVE_REGIONS['url']; ?></code>
</pre>

<h3>Parameters</h3>
<?php foreach ($GEOSERVE_REGIONS['parameters'] as $title=>$info) : ?>
...
<h3>Notes</h3>
<?php foreach ($GEOSERVE_REGIONS['notes'] as $note) : ?>
...
<?php foreach ($GEOSERVE_REGIONS['output'] as $title=>$output) : ?>
...
<ul>
  <?php foreach ($GEOSERVE_REGIONS['examples'] as $example) : ?>
    <li>
